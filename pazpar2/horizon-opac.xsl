...
-->  
  ...
  <xsl:template name="record-hook">
    <xsl:for-each select="/opacRecord/holdings/holding">
	  ...
	  <xsl:when test="circulations/circulation/availableThru">
	    <!-- Winnipeg Public Library (Horizon) puts call number in this field. Weird -->
	    <xsl:value-of select="circulations/circulation/availableThru"/>
	  </xsl:when>
          <xsl:otherwise>PAZPAR2_NULL_VALUE</xsl:otherwise>
        </xsl:choose>
      </pz:metadata>
