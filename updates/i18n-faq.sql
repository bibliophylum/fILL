...
('public/faq.tmpl','fr','js_lang_data','a-sent-home',null,'Le prêt interbibliothèques est une opération effectuée entre deux différents systèmes de bibliothèque; il ne facilite pas un prêt direct entre l’usager d’une bibliothèque locale et un autre système de bibliothèque. Le prêt interbibliothèques offre un accès local aux documents de bibliothèque provenant de toutes les régions de la province. Une fois que l’article est reçu de la bibliothèque de prêt, il doit être traité et votre bibliothèque locale vous informe lorsque vous pouvez le recueillir.'),
('public/faq.tmpl','fr','js_lang_data','q-why-fill',null,'Pourquoi appelle-t-on le système fILL?'),
('public/faq.tmpl','fr','js_lang_data','a-why-fill',null,'« ILL » signifie « interlibrary loan » (prêt interbibliothèques) et « f » est la première lettre de « free and open-source software (FOSS) » (logiciel libre et ouvert). On l’appelle aussi fILL parce qu’il permet à votre bibliothèque locale de remplir (en anglais, fill) facilement votre demande de prêt interbibliothèques en ligne.'),
...
('public/faq.tmpl','fr','js_lang_data','a-on-loan',null,'Votre bibliothèque locale peut demander qu’on retienne l’article. Lorsque l’article est retourné, la demande de prêt interbibliothèques est traitée.'),
