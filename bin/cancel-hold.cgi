...
if (($from == $msg_from) && ($to == $msg_to) && ($status eq 'ILL-Answer|Hold-Placed|estimated-date-available')) {
    $retval = $dbh->do("delete from requests_active where request_id=? and status='ILL-Answer|Hold-Placed|estimated-date-available'",undef,$reqid);
}
