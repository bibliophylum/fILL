...
my $SQL = "select r.id, r.title, r.author, l.library, ra.message from request r left join requests_active ra on r.id = ra.request_id left join libraries l on ra.msg_from = l.lid where r.id=? and ra.status='Shipped'";
