...
my $SQL="select rc.id, rc.title, rc.author, rc.requester, rh.ts, rh.status, rh.message from request_closed rc left join requests_history rh on (rc.id=rh.request_id and rh.msg_from=?) where rc.requester<>? and rh.ts in (select max(ts) from requests_history where msg_from=? group by request_id) and rh.ts >= ? and rh.ts < ? order by ts";
