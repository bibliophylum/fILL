...
my $SQL = "select r.id, r.title, r.author, ra.ts, ra.msg_from, ra.message, ra.status from request r left join requests_active ra on (r.id = ra.request_id) where ra.msg_to=? and ra.status='Returned' and ra.request_id not in (select request_id from requests_active where msg_from=? and status='Checked-in') order by r.author, r.title";
