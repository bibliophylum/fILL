...
#
#    public_original.pm is part of fILL.
#
...
package fILL::public_original;
use warnings;
use strict;
use base 'public_original_base';
use GD::Barcode;
...
sub search_process {
    ...
    my $template;
    if ($is_enabled) {
	$template = $self->load_tmpl('public-original/search.tmpl');
    } else {
	$template = $self->load_tmpl('public-original/not-enabled.tmpl');
    }
...
sub request_process {
	...
	my $template = $self->load_tmpl('public-original/request_placed.tmpl');	
...
sub myaccount_process {
    ...
    my $template = $self->load_tmpl('public-original/myaccount.tmpl');	
...
sub current_process {
    ...
    my $template = $self->load_tmpl('public-original/current.tmpl');	
...
sub registration_process {
    ...
    my $template = $self->load_tmpl('public-original/registration.tmpl');
