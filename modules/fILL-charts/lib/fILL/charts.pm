...
# fILL::charts is unlike the other fILL modules - it does not provide any CGI runmodes.
...
# OR
#    $self->{param}->{stats} = $stats_href from fILL::stats
    ...
    if (not exists $self->{param}{stats}) {
	$self->{statsObj} = fILL::stats->new('oid' => $self->{param}{oid}, 
					     'year' => $self->{param}{year}, 
					     'month' => $self->{param}{month});
	$self->{param}{stats} = $self->{statsObj}->get_stats();
    } else {
	# caller passed in a $stats href.  Pull out the year/month:
	$self->{param}{oid} = $self->{param}{stats}{oid};
	$self->{param}{year} = $self->{param}{stats}{year};
	$self->{param}{month} = $self->{param}{stats}{month};
    }
...
sub _get_borrowing_received {
    my $self = shift;
    my $href = $self->{param}{stats}{borrowing}{we_received}{type};
...
sub _get_borrowing_unfilled {
    my $self = shift;
    my $href = $self->{param}{stats}{borrowing}{requests_unfilled}{type};
...
sub _get_lending_unfilled {
    my $self = shift;
    my $href = $self->{param}{stats}{lending}{responded_unfilled}{status};
