...
function build_rows( tBody, data ) {
	...
	row = tBody.insertRow(-1); row.id = 'oid'+data.libraries[i].oid;
    ...
    $("#oid-1 > td").each(function() {
	$( this ).attr('class','average');
    });

    // self library id number is in $("#oid") in page header:
    $("#oid"+$("#oid").text()+" > td").each(function() {
