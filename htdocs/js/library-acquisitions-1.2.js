...
$('document').ready(function(){
    ...
    $.getJSON('/cgi-bin/get-library-acquisitions.cgi', {oid: $("#oid").text()},
            function(data){
                build_table(data);
...
function build_table( data ) {
    ...
    {
        row = tBody.insertRow(-1); row.id = 'acq'+data.acquisitions[i].oid;
        cell = row.insertCell(-1); cell.innerHTML = data.acquisitions[i].oid;
...
function ajax_clear_acquisitions() {
    $.getJSON('/cgi-bin/clear-acquisitions.cgi', {oid: $("#oid").text()},
	      function(data){
	      })
    .success(function() {
