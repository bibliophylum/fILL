...
function build_table( data ) {
	...
	if (data.noresponse[i].opt_in == false) { // have not opted in for ILL
	    $(rowNode).children(":eq(5)").addClass("ill-status-no");
	    $(rowNode).children(":eq(5)").attr("title",data.noresponse[i].library+" is not open for ILL");
	}
