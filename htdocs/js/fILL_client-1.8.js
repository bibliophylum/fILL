...
function my_onstat(data) {
	...
	if(data.activeclients != 0){
		...
		if(querybox.value != ""){
		    ...
		    pager.innerHTML = "";
		    if (pager2 !== null) { pager2.innerHTML = ""; }
...
function renderDetails(data, marker)
{
	...
	requestForm = '<form action="/cgi-bin/lightning.cgi" method="post" target="_blank"><input type="hidden" name="rm" value="request">';
    } else {
	// if the logged-in library has not opted to restart ILL,
	// the "request" form won't actually initiate a request.
	requestForm = '<form action="javascript:void(0);"';
    }
...
//	}
    ...
    // for handling the case where a shared ILS is searched, but none of the holding libraries are open for ILL:
    var cntSpruceLocAvailable = 0;
    var cntTotalLocations = data["location"].length;  // for the extreme case where Spruce was the only system, AND none of the Spruce libraries with holdings are open.
	...
	if ("Spruce Libraries Cooperative" === data["location"][i]["@name"]) {
		...
		for (var lloc = 0; lloc < data["location"][i]["md-locallocation"].length; lloc++) {
		    cntSpruceLocAvailable++;
		    SpruceLocAvailable[ data["location"][i]["md-locallocation"][lloc] ] = true; // assume true...
			...
			if (window.SpruceClosed.closed[x].z3950_location === data["location"][i]["md-locallocation"][lloc]) {
			    SpruceLocAvailable[ data["location"][i]["md-locallocation"][lloc] ] = false; // ...unless it's on the closed list
			    cntSpruceLocAvailable--;
			}
		    }
		}
	...
	if (("Spruce Libraries Cooperative" === data["location"][i]["@name"]) && (cntSpruceLocAvailable === 0)) {
	    // If we're looking at Spruce, but no Spruce locations holding the title are open for ILL:
	    details += '<tr><td><b>No available locations.</b></td><td><b>:</b> Holdings exist in Spruce, but not at any location which is available for ILL.</td></tr>';
	    cntTotalLocations--;
	} else {
	    if (data["location"][i]["md-locallocation"] != undefined) {
		var delArr = [];
		for (var lloc = 0; lloc < data["location"][i]["md-locallocation"].length; lloc++) {
		    if (("Spruce Libraries Cooperative" === data["location"][i]["@name"]) && (SpruceLocAvailable[ data["location"][i]["md-locallocation"][lloc] ] === false)) {
			// Found location is a Spruce library that is closed, so don't show it.
			console.log('Found at Spruce(' + data["location"][i]["md-locallocation"][lloc] + '), which is closed. Skipping.');
			delArr.push( data["location"][i]["md-locallocation"][lloc] );
		    } else {
			addToRequest = true;
		...
		// remove the closed Spruce local locations, so they don't get added to the requestForm
		for( var x = 0; x < data["location"][i]["md-locallocation"].length; x++){
		    for (var y = 0; y < delArr.length; y++) {
			if ( data["location"][i]["md-locallocation"][x] === delArr[y]) {
			    data["location"][i]["md-locallocation"].splice(x, 1); x--;
			}
		    }
		}
	...
	} else {  // don't give a 'request' button if there are no (open) locations, or if this library has not opted-in for ILL.
	    if (cntTotalLocations <= 0) {
		requestForm += '<p>Holdings exist in Spruce, but not at any location which is currently available for ILL.</p>';
	    } else if (window.ILLStatus.opt_in === false) {
		requestForm += '<p><font style="background-color: yellow;">You will be able to make ILL requests after opting-in to ILL.</font></p>';
	    }
	}
    }
    requestForm += '</form>';
