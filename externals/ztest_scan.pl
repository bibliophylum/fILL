...
my $L4U = 0; # set to 1 to test The Pas

print "test #1 - finding search terms\n";
eval {
    if ($L4U) {
	$conn = new ZOOM::Connection("206.45.107.244:210/Default"); # The Pas Regional Library
    } else {
	$conn = new ZOOM::Connection("198.163.53.31:210/horizon");  # Winnipeg Public Library
    }
...
eval {
	...
	$conn = new ZOOM::Connection("198.163.53.31:210/horizon");
    }
    print("server is '", $conn->option("serverImplementationName"), "'\n");
    $conn->option(preferredRecordSyntax => "usmarc");
    $ss = $conn->scan_pqf('@attr 1=1003 a');
