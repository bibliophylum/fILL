...
for (my $i = 0; $i < @$ar_conn; $i++) {
	...
	$dbh->do( "UPDATE zservers SET alive=0 WHERE id=?", undef, $ar_conn->[$i]{id});

	unless ($ar_conn->[$i]{notification_sent}) {
		...
		$dbh->do( "UPDATE zservers SET notification_sent=1 WHERE id=?", undef, $ar_conn->[$i]{id});
	    }
	}
	...
	unless ($ar_conn->[$i]{alive}) {
	    $dbh->do( "UPDATE zservers SET alive=1 WHERE id=?", undef, $ar_conn->[$i]{id});
		...
		$dbh->do( "UPDATE zservers SET notification_sent=0 WHERE id=?", undef, $ar_conn->[$i]{id});
	    }
	}
